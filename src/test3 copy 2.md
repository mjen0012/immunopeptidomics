...
const seqCalcAll = (
  ...
  .map(r => ({
    ...
    // Assuming your parquet file has a pre-calculated frequency/value column
    // like the 'value' column in your CSV. If not, calculate it here.
    // For this example, I'll assume a column named 'frequency'.
    frequency: Number(r.value) // Using 'value' from your CSV info
  }));

/* helper – canonicalise protein names */
const normProtein = s => s.trim().replace(/\s+/g, "").toUpperCase();

/* aaFreqsByProtein : Map<canonical protein → Array<Map<residue, frequency>>> */
...
for (const { protein, position, aminoacid, frequency } of seqCalcAll) {
  ...
  // Store the frequency for the amino acid at that position
...
```js
/*****************************************************************
 * (Optimized) Banded Needleman-Wunsch with DYNAMIC Band Width
 * --------------------------------------------------------------
 * This version automatically adjusts the band width to handle
 * large differences in sequence vs. profile length.
 *****************************************************************/
function nwAffineBanded(ref, freqs, baseBandWidth = 75, gOpen = -5, gExt = -2) {
  const M = freqs.length, N = ref.length;

  /*****************************************************************
   * ✨ NEW: Dynamic band width calculation ✨
   * We calculate the difference in length and ensure the band is
   * wide enough to accommodate it, adding a small buffer (20).
   *****************************************************************/
  const lengthDiff = Math.abs(M - N);
  const bandWidth = Math.max(baseBandWidth, lengthDiff + 20);

  // The old explicit check is no longer needed, as the band is now guaranteed to be wide enough.
  ...
  Mx[0][0] = 0;
  // Initialize first row and column. The band check isn't strictly necessary
  // here with the dynamic width, but it costs nothing to leave it.
  ...
  /* ---------- Traceback (Corrected and Cleaned) ------------- */
  let aln_b = "";
  let i = M, j = N;
  ...
  while (i > 0 || j > 0) {
    if (state === 0) { // Match/Mismatch
        ...
        state = TBM[i][j];
        i--; j--;
    } else if (state === 1) { // Gap in reference (moves up)
        aln_b = "-" + aln_b;
        state = TBIx[i][j];
        i--;
    } else { // state === 2, Gap in profile (moves left)
        if(j > 0) aln_b = ref[j-1] + aln_b;
        state = TBIy[i][j];
        j--;
    }
...
html`
  <button style="margin-bottom:1em;"
          onclick=${() => {
            /* 1) sanity-check */
            if (!fastaAligned?.length) {
              alert("No aligned-peptide data available.");
              return;
            }

            /* 2) convert to CSV text */
            const csv  = d3.csvFormat(fastaAligned);

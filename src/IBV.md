...
const db = await getOrCreateDB(() =>
  DuckDBClient.of({
    ...
    sequencecalc: FileAttachment("data/IBV_preaggregated_frequencies.parquet").parquet(),
    hla: FileAttachment("data/HLAlistClassI.parquet").parquet()
  })
);

// Allowed IBV tokens (no prefix in the UI)
const ALLOWED_IBV_TOKENS = new Set([
  "BM2","HA","M1","NA","NB","NEP","NP","NS1","PA","PB1","PB2"
]);

// Normalize any incoming id to a plain token (strip optional "IBV_" just in case)
const toToken = (id) => (id?.startsWith("IBV_") ? id.slice(4) : id);
...
// Init once so dependents can run
{
  const token = DEFAULT_PROTEIN_TOKEN;
  ...
  globalThis.__proteinViewState = { lastFileKey: fileKey };
}
...
const proteinOptions = [
  { id: "BM2",  label: "BM2 Protein (BM2)" },
  { id: "HA",   label: "Hemagglutinin (HA)" },
  { id: "M1",   label: "Matrix Protein 1 (M1)" },
  { id: "NA",   label: "Neuraminidase (NA)" },
  { id: "NB",   label: "NB Protein (NB)" },
  { id: "NEP",  label: "Nuclear Export Protein (NEP)" },
  { id: "NP",   label: "Nucleoprotein (NP)" },
  { id: "NS1",  label: "Nonstructural Protein 1 (NS1)" },
  { id: "PA",   label: "Polymerase PA (PA)" },
  { id: "PB1",  label: "Polymerase PB1 (PB1)" },
  { id: "PB2",  label: "Polymerase PB2 (PB2)" }
];
```
...
```js
// Reactive reload — always IBV_*, never falls back to non-prefixed names
{
  // committedProteinId is a TOKEN like "HA" (or defensively "IBV_HA")
  const selected = committedProteinId ?? DEFAULT_PROTEIN_TOKEN; // reactive dep
  const token = toToken(selected); // ensure plain token
  if (!ALLOWED_IBV_TOKENS.has(token)) throw new Error(`Unknown IBV token: ${token}`);

  const fileKey = toIBVFileKey(token); // "IBV_HA", "IBV_NS1", etc.
